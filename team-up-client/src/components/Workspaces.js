...
function Workspaces() {
  ...
  const getWS = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://${process.env.REACT_APP_SERVER_NAME}:4000/users/${userID}/workspace`,
        header
      );
      setWS(data);
    } catch (e) {
  ...
  const deleteWS = async (id) => {
    try {
      ...
      await axios.delete(`https://${process.env.REACT_APP_SERVER_NAME}:4000/workspace/${id}`, header);
      getWS();
    } catch (e) {
      console.log(e);
    }
