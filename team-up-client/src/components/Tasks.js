...
function Tasks() {
  ...
  const getData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `https://${process.env.REACT_APP_SERVER_NAME}/workspace/${id}/tasks`,
        header
      );
      setTaskList(data);
    } catch (e) {
  ...
  const completeTask = async (taskData) => {
    try {
      ...
      await axios.patch(
        `https://${process.env.REACT_APP_SERVER_NAME}/workspace/task/${id}/${taskData._id}`,
        dataa,
        header
      );
      await getData();
  ...
  const deleteTask = async (taskID) => {
    try {
      header = await getHeader();
      await axios.delete(
        `https://${process.env.REACT_APP_SERVER_NAME}/workspace/task/${id}/${taskID}`,
        header
      );
      await getData();
    } catch (e) {
